...
/*
@brief  Linked List를 구현하기 위해서 정의해준 구조체이다.
*/
...
/*
@brief  정렬된 DoublySortedLinkedList..
*/
template <class ItemType>
...
public:
	...
	*	@post	데이터 정보를 검색하며, 함수가 잘 작동되면 true를 아니면 false를 리턴한다.
	*/
	bool search(ItemType &item);
	/**
	*	@brief	입력받은 아이디를 비교하여 찾고, 데이터 정보를 갱신한다.
	*	@pre	갱신받기 위해 입력받은 아이디를 가지고 있어야한다.
	*	@post	데이터 정보를 수정된경우 true를 리턴 아니면 false를 리턴한다.
	*/
